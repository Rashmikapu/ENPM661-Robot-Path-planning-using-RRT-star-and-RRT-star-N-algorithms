...
def rrt_star_n(initial , final , canvas):
    ...
    points = create_gaussian_space(initial,final)
    ...
    while (1):
        ...
        if Zrand[0]>=canvas.shape[0] or Zrand[0]< 0 or Zrand[1]<0 or Zrand[1]>=canvas.shape[1] :
            continue
        ...
        cost =  Euclidean_dist(Znew,initial) + Euclidean_dist(Znew, final) + Euclidean_dist(Znew, Znearest)
        ...
        Td[tuple(Znew)] = Znearest                         #new 1node : parent
...
def create_gaussian_space(point1, point2, num_points= 1000, sigma = 90):
    ...
    # Create an array of x,y-coordinates along the line
    x_coords = x1 + (x2 - x1) * t_values
    
   
    y_coords = y1 + (y2 - y1) * t_values
    
    
    displacements = np.random.normal(0, sigma, num_points)
    dx = x2 - x1
    dy = y2 - y1
    ...
    # Displace the points perpendicular to the line
    x_coords += displacements * unit_dy
    y_coords -= displacements * unit_dx
    # if count==0 :
    #     plt.plot(x_coords, y_coords)
    #     plt.scatter([point1[0], point2[0]], [point1[1], point2[1]], color='red')
    #     plt.xlabel('x')
    #     plt.ylabel('y')
    #     plt.title('Normal Space along the Line')
    #     plt.grid(True)
    #     plt.show()
    #     count = 1
    #i = random.randint(0, len(x_coords)-1)
    # print("Inside Sample")
...
def Backtrack1(closed_list, T, initial_state, final_state, canvas):
    ...
    for key in keys:
        ...
        cv2.imshow("RRT*N Exploration and Optimal Path Visualization",canvas)
        cv2.waitKey(1)
...
if __name__ == '__main__': 
    ...
    i = random.randint(radius, canvas.shape[0]-1-radius)
    j = random.randint(radius, canvas.shape[1]-1-radius)
    #cv2.circle(canvas, (j,i), radius, (0,0,255), -1)
